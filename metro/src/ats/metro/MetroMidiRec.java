...
public class MetroMidiRec implements MetroMidiReceiver<byte[]> {
	public static MetroMidiRec getInstance() {
		return INSTANCE;
	}
	private static final MetroMidiRec INSTANCE = new MetroMidiRec();
	private MetroMidiRec() {
	}
	
	/** @formatter:on */
	static final Logger LOGGER = Logger.getLogger( MetroMidiRec.class.getName() );
	
	public byte[] noteOn( int channel, int note, double velocity ) {
		return MetroMidiMessage.noteOn (channel, note, velocity );
	}
	public byte[] noteOn( int channel, int note, int velocity ) {
		return MetroMidiMessage.noteOn (channel, note, velocity );
	}
	public byte[] noteOff ( int channel, int note, double velocity ) {
		return MetroMidiMessage.noteOff(channel, note, velocity );
	}
	public byte[] noteOff ( int channel, int note, int velocity ) {
		return MetroMidiMessage.noteOff(channel, note, velocity );
	}
	public byte[] keyPressure( int channel, int note, double value ) {
		return MetroMidiMessage.keyPressure( channel, note, value );
	}
	...
	public byte[] controlChange( int channel, int controlNumber, int controlValue ) {
		return MetroMidiMessage.controlChange( channel, controlNumber, controlValue );
	}
	public byte[] programChange( int channel, int value ) {
		return MetroMidiMessage.programChange( channel, value );
	}
	public byte[] channelPressure( int channel, double value ) {
		return MetroMidiMessage.channelPressure( channel, value );
	}
	public byte[] channelPressure( int channel, int value ) {
		return MetroMidiMessage.channelPressure( channel, value );
	}
	public byte[] pitchBend(int channel, double value) {
		return MetroMidiMessage.pitchBend( channel, value );
	}
	...
	public byte[] cc_allSoundOff(int channel) {
		return MetroMidiMessage.cc_allSoundOff( channel );
	}
	public byte[] cc_resetAllControllers(int channel) {
		return MetroMidiMessage.cc_resetAllControllers( channel );
	}
	public byte[] cc_localControls(int channel,boolean value ) {
		return MetroMidiMessage.cc_localControls( channel, value ) ;
	}
	public byte[] cc_allNoteOff(int channel) {
		return MetroMidiMessage.cc_allNoteOff( channel );
	}
	public byte[] cc_omniModeOff(int channel) {
		return MetroMidiMessage.cc_omniModeOff( channel );
	}
	public byte[] cc_omniModeOn(int channel) {
		return MetroMidiMessage.cc_omniModeOn( channel );
	}
	// TODO : Isn't this mono-mode-on? This seems incorrect.  
	public byte[] cc_monoModeOn(int channel) {
		return MetroMidiMessage.cc_monoModeOn( channel );
	}
	public byte[] cc_polyModeOn(int channel) {
		return MetroMidiMessage.cc_polyModeOn( channel );
	}
